...
class robot:
    ...
    def sense(self):
        ''' Measure (dx, dy) to every landmark that is within `measurement_range`
            of the robot's current (x, y) position. Each measurement gets a
            small noise component drawn from [-measurement_noise, measurement_noise).
            Returns a list of [landmark_index, dx, dy].
        '''
        measurements = []
        for i, (lx, ly) in enumerate(self.landmarks):
            ...
            # measurement_range == -1 means "see everything"
